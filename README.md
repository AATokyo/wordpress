# Self-Hosted WordPress with PostgreSQL, MinIO, Redis, and Nginx Proxy Manager

This stack runs WordPress entirely in Docker with self-hosted PostgreSQL, MinIO, Redis, and Nginx Proxy Manager.
...
After that, access WordPress through `http://wp.localtest.me`. If you also proxy MinIO, uploaded media will be served from `http://uploads.localtest.me/<bucket>`.
...
PostgreSQL, MinIO, Redis, and Nginx Proxy Manager are all included in this Compose stack. No external cloud database or object storage service is required.

The image installs `pdo_pgsql` and wires in a `db.php` drop-in plus the S3 Uploads plugin for MinIO-compatible object storage.
